#!/usr/bin/env python3
"""
analisis_semana3.py — Metricas semana 3.
fraccion_activa se lee directamente del HDF5 (nativa desde capturar.py).
raw_signal solo se lee para 1 representante por condicion (timeline).

Uso:
  python3 analisis/analisis_semana3.py --dir capturas/semana3
"""
import argparse
import sys
...
OUTPUT_DIR = Path(__file__).parent / 'outputs_semana3'
...
# ---------------------------------------------------------------------------
# Lee raw_signal solo para 1 representante por condicion (timeline)
# ---------------------------------------------------------------------------
def cargar_kurts_para_timeline(datos: list[dict]):
    ...
    print(f'\nCargando raw_signal para timeline ({len(representantes)} archivos):')
    for d in representantes:
        fs        = d['fs']
        nyq       = fs / 2.0
...
def plot_boxplots(datos: list[dict], out_dir: Path):
    ...
    fig.suptitle('Metricas por condicion — semana 3',
                 fontsize=12, fontweight='bold')
    plt.tight_layout()
    out = out_dir / 'boxplots_semana3.png'
...
def plot_timeline(datos: list[dict], out_dir: Path):
    representantes = [d for d in datos if 'kurts_por_ventana' in d]
    representantes.sort(key=lambda d: ORDEN.index(d['condicion']) if d['condicion'] in ORDEN else 99)
    ...
    for ax, d in zip(axes, representantes):
        ...
        n_vent   = len(kurts)
        t_centro = (np.arange(n_vent) + 0.5) * VENTANA_S
...
def plot_scatter_masa(datos: list[dict], out_dir: Path):
    ...
    for ax, met, label, use_log in zip(axes, METS, LABELS, LOG):
        ...
        ax.set_xticks(sorted(np.unique(masas).tolist()))
    ...
    fig.suptitle('Metricas vs masa de arena — semana 3', fontsize=12, fontweight='bold')
    plt.tight_layout()
    out = out_dir / 'scatter_masa_semana3.png'
    plt.savefig(out, dpi=150, bbox_inches='tight')
    plt.close()
    print(f'  Guardado: {out.name}')
...
def main():
    ...
    p.add_argument('--dir', default='capturas/semana3',
                   help='Directorio con archivos .h5 (default: capturas/semana3)')
    args = p.parse_args()
    ...
    cargar_kurts_para_timeline(datos)
