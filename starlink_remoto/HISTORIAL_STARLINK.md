...
| Asunción de IP pública resulta ser CGNAT | **Descartado (2026-07-31, reconfirmado 2026-08-03):** confirmada IP pública directa sin CGNAT, dos veces con Starlink real conectado (`ifconfig.me`/`api.ipify.org` devuelven la misma IP usada para SSH entrante). |
| Login SSH por password `root`/`root` expuesto en la IP pública de Starlink, sin CGNAT de por medio | **Mitigado (2026-08-03):** password de `root` cambiado, login SSH pasado a solo clave (`PasswordAuthentication no`, `PermitRootLogin prohibit-password` en `/etc/ssh/sshd_config`, backup del original en `sshd_config.bak.20260803_175532`), `fail2ban` instalado sobre `sshd` (backend `systemd`, `maxretry=5`), logs de auth con persistencia acotada entre reboots (journald `Storage=persistent` + `SystemMaxUse=50M`, redirigido fuera del `tmpfs` de `/var/log` vía bind mount en `/etc/fstab` sobre `/var/lib/journal-persist`, porque `/var/log` es tmpfs a propósito para no desgastar la SD). Verificado en vivo: login por clave funciona, login por password rechazado. **Persistencia de logs confirmada con reboot real (2026-08-03, sesión posterior):** marca dejada antes de reiniciar, sigue presente después. VPN (Tailscale/WireGuard) descartada — el usuario decidió no usarla. **Corrección importante (2026-09-10): el `fail2ban` instalado acá NUNCA baneó nada de verdad** — el kernel de esta placa no soporta `nftables` (su `banaction` por defecto), así que cada intento de baneo fallaba en silencio (solo quedaba en `/var/log/fail2ban.log`, nadie lo miraba). Fix real y verificado, con el paso a paso para replicarlo en placas nuevas: ver "Hardening de SSH" en Historial de cambios y `setup_placa.md` paso 4b. `bantime` quedó en 600s (10 min), no 1h, como red de seguridad extra. |
...
- ~~Asunción de IP pública de Starlink sin CGNAT, a reconfirmar en sitio con el kit real conectado~~ — **cerrado:** confirmada IP pública directa sin CGNAT, dos veces (sec.88 y esta sesión), ver "Riesgos abiertos" arriba.
- ~~Mitigar el login SSH `root`/`root` expuesto en la IP pública de Starlink~~ — **cerrado (2026-08-03):** ver fila en "Riesgos abiertos".
...
### Hardening de SSH en la placa de Starlink (2026-08-03, misma sesión)

**Motivo:** cierra el hallazgo de seguridad abierto desde la sección de pulsos espurios (2026-07-31) — login `root`/`root` por password expuesto en la IP pública de Starlink (`153.67.6.182`), sin CGNAT de por medio, pospuesto en su momento para no mezclar riesgos con las pruebas del relé.

**Diagnóstico previo, dos cosas que no eran lo que parecían:**
- `/etc/ssh/sshd_config.d/` está **vacío** — no hay ningún drop-in pisando nada. `PermitRootLogin yes` estaba explícito en la línea 132 del `sshd_config` principal; `PasswordAuthentication` no estaba seteado en ningún lado y corría con el default de OpenSSH (`yes`).
- El journal de `sshd` **sí tiene entradas** ahora (a diferencia de lo observado en la sesión de 2026-07-31) — pero la razón real de por qué se pierden en cada reboot no es que journald esté en modo volátil por default: **`/var/log` está montado como `tmpfs` (`size=5120k`)** a propósito, casi seguro para no desgastar la SD (`/dev/mmcblk0p2`). Cualquier archivo bajo `/var/log`, journal o no, se pierde en cada reboot sin importar la config de journald.

**Orden de ejecución real (distinto del orden numerado original, porque el paso "investigar dónde vive la directiva" tenía que ir antes de tocarla):**
1. Se copió la clave pública `id_ed25519` existente (esta misma máquina) al `authorized_keys` de `root`, y se confirmó login solo por clave (`BatchMode=yes`, sin fallback a password) **antes** de tocar cualquier config — para no arriesgar un lockout total en una placa sin VPN y en un patio sin acceso físico rápido.
2. Password de `root` cambiado.
3. `sshd_config`: `PermitRootLogin prohibit-password` + `PasswordAuthentication no` (backup del original en `sshd_config.bak.20260803_175532`). Validado con `sshd -t` antes del restart. Tras el restart, se confirmó en el acto que el login por clave seguía funcionando y que el login por password quedaba rechazado (`Permission denied (publickey)`).
4. `fail2ban` instalado (no estaba) y configurado con `backend = systemd` (porque no hay `/var/log/auth.log`, solo journal) — jail de `sshd` activo, `maxretry=5`, `bantime=1h`.
5. Logs de auth con persistencia acotada: como `/var/log` es tmpfs a propósito, no alcanza con `Storage=persistent` de journald solo — el propio `/var/log/journal` se perdería en cada boot igual. Se creó `/var/lib/journal-persist` (en el ext4 real, fuera del tmpfs) y un **bind mount en `/etc/fstab`** (`/var/lib/journal-persist /var/log/journal none bind,nofail,x-systemd.makedirs 0 0`) que monta ese directorio real sobre `/var/log/journal` en cada boot, más `Storage=persistent` y `SystemMaxUse=50M` en un drop-in de journald — capado a propósito para no repetir el mismo problema de desgaste de SD que motivó el tmpfs en primer lugar. **Corrección (2026-08-03, sesión posterior):** el mecanismo real verificado en la placa es este bind mount por fstab, no el symlink vía `/etc/tmpfiles.d/` mencionado originalmente aquí — logra el mismo efecto (sacar `/var/log/journal` del tmpfs) pero la implementación documentada al momento de escribir esta sección no coincidía con lo que quedó realmente configurado.
...
### Falla del pulso al bootear confirmada como determinística (5/5, incluye corte de energía real) + contador de fallos consecutivos y alerta (2026-08-13)

**Contexto:** venía abierta la duda de si la falla del pulso de relé al bootear (visible en 3 reboots forzados seguidos, con degradación de la autorecuperación: 1m39s → 11min → sin recuperación, resuelto a mano con el botón físico) era un problema real de HW o un artefacto de encadenar reboots sin descanso. Se hizo la prueba pendiente: conexión directa a la placa (bypass de Starlink, para no depender del propio mecanismo bajo prueba para mantener el acceso).

**Resultado — 5/5 reboots con la misma falla inicial, recuperación rápida cuando está aislado:**
- Reboot forzado aislado (`systemctl reboot --force --force`, ~24hs sin desenchufar desde la cadena anterior): mismo fallo ("se pidió 'on' pero el feedback sigue en 'off'"), recuperación en el primer tick del reconciliador (~2 min).
- Corte de energía real (desenchufar/reenchufar de verdad, no solo reboot por software): mismo resultado exacto.
- FPGA (v0.94) descartada de nuevo como causa en ambos casos (ya cargada varios segundos antes del pulso fallido).
- Conclusión: la falla inicial es determinística (glitch conocido de habilitar el mux de PS_MIO10 por primera vez tras cada boot), no un artefacto de estrés. La degradación de la recuperación sí parece ser artefacto de encadenar reboots sin descanso — con descanso, la recuperación es rápida siempre.

**Mitigación agregada — contador de fallos consecutivos + alerta (`control_starlink.sh`):** claves nuevas `starlink.alerta_fallos_consecutivos` (4, ~20 min de fallos sostenidos del reconciliador) y `rutas.fallos_consecutivos_file`. Cada verificación resetea el contador a 0 si coincide con el objetivo, lo incrementa si no — al cruzar el umbral, encola un aviso en `avisos_pendientes/` (mismo mecanismo que ya usa `capturar_stream.py`, sin código de red nuevo), una sola vez por episodio de falla sostenida.

**Limitación real, aclarada antes de implementar:** para el caso "falla en confirmar 'on'" (el investigado en esta sesión), el aviso NO sale en el momento — el único camino de red de la placa es el propio Starlink que está caído. Es un registro retrospectivo (se manda solo cuando la conexión vuelve) para ese caso, y un aviso en vivo real para el caso contrario ("falla en confirmar 'off'", donde la red sigue arriba). No es un SOS en tiempo real para el peor escenario — limitación aceptada explícitamente por el usuario, no un defecto de la implementación.

**Validado en HW real** (feedback `DIO2_P` desconectado a mano, mismo truco que la sección de pulsos espurios de arriba): 3 fallos seguidos sin avisar, 4to fallo dispara exactamente un aviso (`{"evento": "rele_sin_confirmar", "objetivo": "on", "fallos_consecutivos": 4}`), 5to no repite. Reconectado el feedback, un llamado exitoso resetea el contador a 0 y confirma el relé en el estado real correcto. El aviso de prueba quedó encolado sin enviar por `HTTP 429` (rate-limit del endpoint de prueba `webhook.site`, no un bug) — confirma que el mecanismo no pierde el archivo si el POST falla.

**Pendiente sin resolver:** la otra dirección posible (acortar la duración del corte en sí, reintento con backoff en `asegurar_mux_ps10.sh` en vez de esperar al reconciliador) quedó descartada por ahora a favor de esta alerta — no se probó si un reintento inmediato al bootear ayuda o no.

### Confirmación doble en el reconciliador de 5 min, para no pisar el botón físico de emergencia (2026-08-20)

**Problema:** el botón físico de emergencia (agregado sec.117 del proyecto) togglea el relé sin que el software se entere. Si se usa fuera de la ventana `hora_on`-`hora_off`, el reconciliador de 5 min lo detecta como desacuerdo contra el horario y lo revierte en el próximo ciclo — hasta 5 min después, sin excepción. El único camino para "salvar" ese encendido es entrar por SSH (que depende de que el propio Starlink recién encendido ya haya levantado, ~2-3 min) y correr `prender-starlink` para fijar modo manual antes de que el reconciliador lo pise. Con el margen real (adquisición de señal + tiempo de conectarse) rondando los 4 min, el margen de 5 min del reconciliador era ajustado, no confiable.

**Solución — confirmación doble, acotada solo al reconciliador:** `control_starlink.sh` acepta un flag `--reconciliar`. Con el flag, si detecta un desacuerdo, no pulsa de una: escribe una marca (`rutas.reconciliador_pendiente_file`) con el objetivo pedido y sale sin tocar el HW. Recién si el mismo objetivo sigue en desacuerdo en el ciclo siguiente (~5 min después), confirma y pulsa — peor caso ~10 min con el relé en el estado "equivocado", suficiente margen para fijar modo manual. Los timers `starlink-rele-on.timer`/`-off.timer` (horario exacto) y los comandos manuales **no** pasan el flag — corrigen de inmediato como siempre, sin ningún retraso. Se creó un service nuevo (`starlink-reconciliador.service`, con `ExecStart ... --reconciliar`) separado de `starlink-aplicar-objetivo.service` (que siguen usando los timers de horario) porque los tres timers compartían la misma unidad y no había forma de pasarle un flag distinto solo a uno.

**Se descartó aumentar el intervalo del reconciliador (la otra opción considerada):** afecta también el tiempo de detección/alerta de una falla real de pulso (`alerta_fallos_consecutivos`, ver sección de arriba) — con 15 min de intervalo, el umbral de 4 fallos consecutivos tarda 60 min en dispararse en vez de 20. La confirmación doble no toca ese camino (solo pospone antes de intentar pulsar, nunca antes de reintentar tras un pulso ya fallido).

**Validado en HW real, sin querer, con un corte de conexión real de ~13 min:** una prueba manual (`control_starlink.sh off --reconciliar` llamado dos veces seguidas) apagó el relé de verdad — cortando la única conexión (Starlink) usada para la prueba. La recuperación automática confirmó las dos mitades del mecanismo en el log: ciclo de las 14:43 UTC detectó el desacuerdo (pidió 'on', HW en 'off') y pospuso; ciclo de las 14:48 UTC confirmó y pulsó ("rele ahora en 'on', confirmado por HW"). **Lección para la próxima prueba de este tipo:** no generar el desacuerdo con dos llamadas manuales seguidas contra la única conexión en uso — cualquier prueba de un pulso real de "off" corta el único camino de acceso a la placa (ver [[reference_placa_banco]]).

**Pendiente sin probar:** el caso que más importa del diseño — que fijar modo manual (`prender-starlink`/`apagar-starlink`) a mitad de la ventana de espera corte el ciclo de inmediato en vez de esperar el segundo tick. Por código tiene que funcionar (en cuanto el objetivo calculado coincide con el HW, `control_starlink.sh` sale por el atajo de "ya está" y borra cualquier marca pendiente, sea cual sea el flag), pero no se verificó en vivo — el usuario decidió dejarlo así, confiando en la lectura del código en vez de arriesgar otro corte real. Si se retoma, probarlo cerca de un corte de horario ya esperado (ej. justo antes de `hora_off`), no en medio de una ventana de uso real.

### `fail2ban` nunca baneó nada de verdad — el kernel de esta placa no soporta `nftables` (2026-09-10)

**Cómo se encontró:** investigando por qué un ataque de fuerza bruta SSH venía siendo tan intenso (miles de intentos fallidos en horas, ver memoria del proyecto sec.160/161), se comparó `fail2ban-client status sshd` (decía IPs "banned") contra `iptables -L INPUT` real (siempre vacío). `/var/log/fail2ban.log` tenía, desde el día de la instalación (sec.93, 2026-08-03), un error en CADA intento de baneo: `nft: not found`. El `banaction=nftables` que trae `fail2ban` por defecto en Debian nunca funcionó en esta placa porque el kernel (`6.12.0-redpitaya`, build custom para Zynq) tiene `CONFIG_NF_TABLES is not set` — no soporta `nftables` en absoluto, ni siquiera vía la capa de compatibilidad `iptables-nft` (`iptables: Failed to initialize nft: Protocol not supported`). **Conclusión: desde que se instaló, ningún IP atacante fue bloqueado de verdad — todo el tráfico de fuerza bruta entró sin ningún freno real.**

**Por qué esto puede pasar en cualquier placa Red Pitaya/Zynq, no es un accidente de esta placa puntual:** son kernels embebidos, compilados a medida, sin todos los módulos de un kernel de escritorio/servidor típico. Además de `nftables`, tampoco están: el módulo `multiport` de `iptables` (`CONFIG_NETFILTER_XT_MATCH_MULTIPORT is not set` — rompe `banaction=iptables-multiport`, la otra opción común) y el target `REJECT` (`CONFIG_IP_NF_TARGET_REJECT is not set` — rompe el `blocktype` por defecto de las acciones de `iptables`, que es `REJECT --reject-with icmp-port-unreachable`). Lo que SÍ está disponible y funciona: el framework legacy `ip_tables`/`xtables-legacy` completo, con los targets básicos `DROP`/`ACCEPT`/`RETURN` y el match de puerto simple (`--dport`, sin `multiport`).

**Fix aplicado y verificado en HW real (placa `rp-f0fbda`, ver [[project_sand_monitoring]] sec.164):**
1. `update-alternatives --set iptables /usr/sbin/iptables-legacy` (+ `ip6tables` igual) — el binario `iptables` del sistema apuntaba por defecto a `xtables-nft-multi`, inútil en este kernel.
2. `/etc/fail2ban/jail.local`:
   ```
   [DEFAULT]
   ignoreip = 127.0.0.1/8 ::1 <IP de confianza que nunca se quiere banear>
   banaction = iptables
   ...
   bantime = 600
   ```
   Puntos no obvios: `banaction = iptables` (NO `iptables-multiport`, rompe por el módulo faltante). `blocktype = DROP` sí o sí — un `blocktype=DROP` suelto en `[DEFAULT]` no alcanza, hay que sobreescribir `action_` para que lo interpole y se lo pase de verdad a la acción (si no, la acción sigue usando su default `REJECT` interno y falla igual). `bantime` bajado de 1h a 600s (10 min) a propósito, como red de seguridad: si el `ignoreip` alguna vez queda desactualizado (ej. cambia la IP pública de quien lo puso) y esa persona se autobanea sin querer, el bloqueo se libera solo en minutos en vez de una hora — la placa de campo no tiene acceso físico rápido.
3. Reiniciar `fail2ban` y verificar con una IP de prueba inofensiva (rango de documentación `203.0.113.0/24`, RFC 5737, nunca se conecta de verdad): `fail2ban-client set sshd banip 203.0.113.1` → confirmar la regla `DROP` en `iptables -L f2b-sshd -n -v` → `fail2ban-client set sshd unbanip 203.0.113.1` → confirmar que desaparece. Confirmar en paralelo que la sesión SSH propia sigue viva durante todo el proceso.

**Aplicar esto de entrada en cualquier placa nueva que instale `fail2ban`** (ver `setup_placa.md` paso 4b) — no asumir que "está instalado y activo" significa que banea de verdad; hay que verificarlo con la prueba del punto 3 la primera vez.
