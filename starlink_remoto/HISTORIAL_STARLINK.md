...
| Login SSH por password `root`/`root` expuesto en la IP pública de Starlink, sin CGNAT de por medio | **Sin mitigar.** Pospuesto a pedido del usuario para priorizar las pruebas del relé (sec.88) — esas pruebas ya se completaron (sec.89/90/91) y esto sigue exactamente igual. Pendiente: cambiar password, pasar a auth por clave, `fail2ban`, revisar logging de auth. |
...
- Mitigar el login SSH `root`/`root` expuesto en la IP pública de Starlink (ver fila nueva en "Riesgos abiertos") — pospuesto desde sec.88, sigue sin tocarse.
