...
5. Logs de auth con persistencia acotada: como `/var/log` es tmpfs a propósito, no alcanza con `Storage=persistent` de journald solo — el propio `/var/log/journal` se perdería en cada boot igual. Se creó `/var/lib/journal-persist` (en el ext4 real, fuera del tmpfs) y un `/etc/tmpfiles.d/journal-persist.conf` que recrea el symlink `/var/log/journal → /var/lib/journal-persist` en cada boot, más `Storage=persistent` y `SystemMaxUse=50M` en un drop-in de journald — capado a propósito para no repetir el mismo problema de desgaste de SD que motivó el tmpfs en primer lugar. Verificado tras flush: journald deja de usar el runtime journal de `/run` y escribe en `/var/log/journal` (resuelto por symlink), con los archivos reales visibles en `/var/lib/journal-persist/<machine-id>/`.

**VPN (Tailscale/WireGuard) descartada explícitamente por el usuario** — no forma parte de este hardening.

**Pendiente real que queda de este trabajo:** no se validó la persistencia de logs con un reboot real todavía (el symlink vía tmpfiles.d es el patrón estándar para este escenario, pero no se forzó un reboot en esta sesión para confirmarlo end-to-end, a diferencia de otros mecanismos de esta placa que sí se validaron con reboot real).
