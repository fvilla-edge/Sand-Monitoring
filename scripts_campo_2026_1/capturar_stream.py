...
Portado desde scripts_campo/capturar_stream.py por el cambio de API del
cliente de streaming: antes un unico ADCStreamClient hacia config+datos;
ahora son dos objetos separados — ConfigStreamClient (conexion, sendConfig,
arranque/parada) y ADCStreamClient (solo datos, recibe el ConfigStreamClient
en el constructor). El evento de fin de chunk (antes 'stoppedSDDone' en el
callback unico) vive ahora en un ConfigCallback colgado del ConfigStreamClient,
no en el ADCCallback de datos. `receivePack` ya no tiene el typo viejo
('recievePack') y no hace falta __disown__() manual al setear el callback.

DISEÑO DISTINTO al viejo en un punto clave: el cliente (confObj/adcObj) se
crea UNA VEZ por sesion, no se recrea en cada chunk. La version vieja
recreaba el cliente por chunk como mitigacion del Bug 1 (race condition);
probado en placa (2026-07-07) que esa mitigacion, portada tal cual a la API
nueva, causaba ELLA MISMA un SIGSEGV al reconectar — un baseline sin esa
mitigacion corrio 3 chunks seguidos limpio. Ver docstring de _nuevo_cliente.
...
muestras por canal (982.068 de 233M en 60s). A dec=64 no se midio perdida.
Se deja --decimacion configurable igual que con 1 canal, pero se avisa si
con 2 canales se elige un valor que no llego a probarse sin perdidas. Estos
numeros son de la placa/ecosistema viejo — pendiente re-medir en 2026.1.
...
def _capturar_chunk(confObj, adcObj, n_muestras, fs_ef, chunk_num, condicion, session_ts, canales, log_evento):
    ...
    El evento de fin de chunk llega por dos vias distintas segun la API
    nueva: datos/errores de transporte via ADCCallback (colgado de adcObj),
    y el resultado del comando de parada (SD lleno, memoria, fin OK) via
    ConfigCallback (colgado de confObj) — antes ambos vivian en un unico
    callback del ADCStreamClient viejo.
    """
    ...
    class Config_CB(streaming.ConfigCallback):
        # adcServerStopped (sin 'code', a diferencia del 'stopped' viejo) se
        # dispara como aviso generico de "todavia no arranco" ANTES de
        # adcServerStartedSD/FPGA (confirmado por trace, 2026-07-07) — no es
        # el fin del chunk. Tratarlo como terminal causaba un done.set()
        # prematuro que llevaba a reconectar un cliente nuevo mientras el
        # streaming-server todavia escribia el chunk anterior — el mismo
        # tipo de race condition que el Bug 1 del sistema viejo.
    ...
    # Sacar los callbacks de este chunk antes del proximo — el cliente
    # (confObj/adcObj) es UNICO para toda la sesion (ver _nuevo_cliente),
    # asi que sin este cleanup se acumularian callbacks viejos escuchando
    # eventos de chunks futuros. removeCallback() de ADCStreamClient no
    # toma argumentos; el de ConfigStreamClient si (asimetria confirmada
    # en placa, 2026-07-07).
...
def _nuevo_cliente(args):
    ...
    adcObj (ADCStreamClient) — UNA VEZ por sesion, no por chunk.

    La version vieja recreaba el cliente en cada chunk como mitigacion
    provisoria del Bug 1 (race condition del lado del streaming-server).
    En 2026.1 esa mitigacion se probo (baseline sin ella) y se confirmo
    lo contrario: reusar el MISMO cliente para varios chunks seguidos
    corre limpio (3 chunks, sin crash, tamaños de archivo correctos),
    mientras que recrearlo entre chunks producia un SIGSEGV justo en la
    reconexion (probado en placa, 2026-07-07). No se porto la mitigacion
    vieja a este cliente nuevo.
    """
...
def main():
    ...
    if args.canales == 2 and args.decimacion not in DEC_SEGURAS_DUAL:
        ...
        cc.log('WARNING', f'      Medido (placa/ecosistema viejo): dec=32 sostenido (60s) perdio ~0.42% de muestras por canal (982.068/233M).')
        cc.log('WARNING', f'      Medido (placa/ecosistema viejo): dec=64 sostenido (60s) — 0 perdidas.')
